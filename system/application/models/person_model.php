...
class Person_model extends Model {
    ...
    function get_most_popular_person($session,$type) {
        ...
        if ($type == 'rep') {
            $sql = 'SELECT p.*
                FROM people p 
                WHERE p.id = (select rep_person_id from most_popular where id = 1)';
        }else {
            $sql = 'SELECT p.*
                FROM people p
                WHERE p.id = (select sen_person_id from most_popular where id = 1)';
        }
    ...
    function get_Person_By_ID($id) {
        ...
        $sql = 'select *,(select description from districts where type = case title when "Rep." Then "h" Else "s" end and district = district_number) as district_description from people where id = ?';
    ...
    function insert_person_view($ip,$person_id) {
        ...
        $sql = "UPDATE page_views SET viewed_on = CURRENT_TIMESTAMP WHERE ip = ? AND person_id = ?";
    ...
    function get_person_contributions($imsp_candidate_id=NULL) {
        
        $xmlfile='http://api.followthemoney.org/candidates.top_contributors.php?key=db01f5c336744450be4a586c44fee304&imsp_candidate_id='.$imsp_candidate_id;
